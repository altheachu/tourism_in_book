...
app.get("/changeTab", async (req, res) => {
  try {
    ...
    if (currentTab === "text"){
      ...
      "amazon_url, isbn, isbn||'-'||$1||'.jpg' as img_name, title, location, " + 
      ...
      const result = await db.query(querySql, [imgSize]);
      const rtnObj = {
...
app.post("/thought", async(req, res) => {
  try {
    ...
    "TO_CHAR(read_date, 'YYYY-MM-DD') as read_date_str from records r inner join " +
    "(select b.id as tmp_book_id, b.isbn, b.title, c.country_name || ', ' || b.area as location " +
    "from books b inner join countries c on b.country = c.country_code where b.country = $2) t " + 
    "on t.tmp_book_id = r.book_id order by t.title";
    const result = await db.query(querySql, [imgSize, countryCode]);
    const rtnObj = {
      articles: result.rows,
      bookSortOptions: bookSortOptions,
      countryNames: countryNames,
...
app.post("/searchArticles", async (req, res) => {
  ...
  let sqlParams = [imgSize];

  if(title || country){
    querySql += "where ";
    
    if(title) {
      querySql += "b.title like '%'||$2||'%' "
    ...
    if(country){
      querySql += "and c.country_name like '%'||$3||'%' "
...
app.get("/note/:id", async (req, res) => {
  ...
  const querySql = "select *, isbn||'-'||$1||'.jpg' as img_name from books b inner join " + 
  ...
  "on n.id = r.note_id where n.id = $2) t on b.id = t.book_id";
  const result = await db.query(querySql, [imgSize, note_id]);
  let article;
...
app.get("/img/:imgName", async (req, res) => {
  try {
    const imgUrl = `${API_URL}${req.params.imgName}` 
    // 內部圖片，sendFile可以發送各種類型文件，包含json跟html
